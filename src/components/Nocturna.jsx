...
const Nocturna = () => {
  ...
  const fetchAvailableParkings = async () => {
    try {
      const endpoints = [
        '/api/disponibilidad_parqueo?JOR_TIPO=NOCTURNA&SECCION=E',
        '/api/disponibilidad_parqueo?JOR_TIPO=NOCTURNA&SECCION=A',
        '/api/disponibilidad_parqueo?JOR_TIPO=NOCTURNA&SECCION=V'
      ];
